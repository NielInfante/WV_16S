...
plot_bar(ps.top20, x="SampleID", fill="Phylum") + facet_wrap(~Treatment, scales="free_x")+geom_bar(stat="identity")
...
metaColor <- read.table('meta2', header=T, stringsAsFactors = F)
...
heatmap.2(otu, trace="none", col = bw, margin=c(6, 6), density.info = 'none',
					RowSideColors = metaColor$Color, labCol=F, key.title=NA, key.xlab='Log Abundance', cexRow = 1.7)
dev.off()
